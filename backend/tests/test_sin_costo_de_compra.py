...
from tests.test_rentabilidad import (  # noqa: F401 — fixtures
    CATEGORIA_ML_PRUEBA,
    _producto_con_precio_y_costo,
    a_store,
    client,
    db_session,
)
...
def _configurar_ml(client, **extra):
    body = {"commission_pct": 15.0, "other_fixed_cost": 0, "min_margin_pct": 15, "min_profit_clp": 3000, "target_margin_pct": 30}
    body.update(extra)
    assert client.put("/api/configuracion/canales/mercadolibre", json=body).status_code == 200


def _envio_de(db_session, tienda, *, precio, costo=5000):
    """El envío de estos casos lo informa Mercado Libre: desde el 16 de
    septiembre de 2026 el envío manual de Configuración no completa el
    cálculo (regla estricta del dueño)."""
    _envio_ml_resuelto(db_session, tienda, categoria=CATEGORIA_ML_PRUEBA, precio=precio, costo=costo)
...
def test_caso_a_con_costo_usa_la_logica_normal(client, db_session, a_store):
    ...
    _envio_de(db_session, a_store, precio=80000)
